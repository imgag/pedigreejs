...
(function (ptree, $, undefined) {
	...
	ptree.build = function (options) {
		var opts = $.extend({ // defaults
			...
			zoomIn: 1.0,
			zoomOut: 1.0,
			diseases: [{ 'type': 'breast_cancer', 'colour': 'darkgrey' },
			{ 'type': 'breast_cancer2', 'colour': 'darkgrey' },
			{ 'type': 'ovarian_cancer', 'colour': 'darkgrey' },
			{ 'type': 'pancreatic_cancer', 'colour': 'darkgrey' },
			{ 'type': 'prostate_cancer', 'colour': 'darkgrey' }],
		...
		var pienode = node.selectAll("pienode")
			.data(function (d) {     		// set the disease data for the pie plot
				...
				var diseases = $.map(opts.additional_diseases, function (val, i) {
					if (prefixInObj(opts.additional_diseases[i].type, d.data)) { ncancers++; return 1; } else return 0;
				})
				if (ncancers === 0) cancers = [1];
				var cancer_data = $.map(cancers, function (val, i) {
					return {
				...
				var disease_data = $.map(diseases, function (val, i) {
					return {
						'cancer': val, 'ncancers': ncancers, 'id': d.data.name,
						'sex': d.data.sex, 'proband': d.data.proband, 'hidden': d.data.hidden,
						'affected': d.data.affected,
				...
				return [$.merge(cancer_data, disease_data)];
			})
			.enter()
			.append("g");
		...
		var warn = node.filter(function (d) {
			return (!d.data.age || !d.data.yob) && !d.data.hidden || d.data.sex == 'U';
		}).append("text")
			.attr('font-family', 'FontAwesome')
			.attr("x", "1em")
			.attr("y", -(0.6 * opts.symbol_size))
			.html("\uf071");
		...
		for (var i = 0; i < opts.diseases.length; i++) {
			...
			addLabel(opts, node, ".25em", -(1.5 * opts.symbol_size),
				function (d) {
					var y_offset = (d.y_offset ? d.y_offset + font_size : font_size * 2.75);
					...
					last_y_offset = (last_y_offset <= y_offset ? y_offset : last_y_offset);
					return y_offset;
				},
				function (d) {
					var dis = disease.replace(/_/g, ' ').replace('cancer', 'ca.');
					return disease + '_diagnosis_age' in d.data ? dis + ": " + d.data[disease + '_diagnosis_age'] : '';
				}, 'indi_details');
		}

		for (var i = 0; i < opts.additional_diseases.length; i++) {
			var disease = opts.additional_diseases[i].type;
			addLabel(opts, node, ".25em", -(1.5 * opts.symbol_size),
				function (d) {
					var y_offset = last_y_offset + font_size;
					for (var j = 0; j < opts.additional_diseases.length; j++) {
						if (disease === opts.additional_diseases[j].type)
							break;
						if (prefixInObj(opts.additional_diseases[j].type, d.data))
							y_offset += font_size - 1;
					}
					return y_offset;
				},
			...
			for (var p = 0; p < opts.dataset.length; p++) {
				if (!p.hidden) {
					if (opts.dataset[p].mother || opts.dataset[p].father) {
						...
						if (midx === -1)
							throw create_err('The mother (IndivID: ' + mother + ') of family member ' + display_name + ' is missing from the pedigree.');
						if (fidx === -1)
							throw create_err('The father (IndivID: ' + father + ') of family member ' + display_name + ' is missing from the pedigree.');
						if (opts.dataset[midx].sex !== "F")
							throw create_err("The mother of family member " + display_name + " is not specified as female. All mothers in the pedigree must have sex specified as 'F'.");
						if (opts.dataset[fidx].sex !== "M")
							throw create_err("The father of family member " + display_name + " is not specified as male. All fathers in the pedigree must have sex specified as 'M'.");
					}
				}
	...
	ptree.rebuild = function (opts) {
		...
		pedcache.add(opts);
		try {
			var validated = dataset_validation(opts.dataset);
			if (validated[0]) {
				ptree.build(opts);
			} else {
				$('#crash_msg').dialog({
					...
					height: 215,
					width: 400,
					dialogClass: 'input-info-dialog',
					buttons: [{
						text: "OK",
						click: function () { $(this).dialog("close"); }
					}]
				});
				$('#crash_msg').html("<p>" + validated[1] + "<br><br>The current change won't be saved.</p>");
				$('#crash_msg').dialog('open');
	...
	ptree.delete_node_dataset = function (dataset, node, opts, onDone) {
		...
		try {
			...
			utils.messages('Warning', 'The deletion of this pedigree member is disallowed.');
			throw err;
		}
		if (unconnected.length > 0) {
